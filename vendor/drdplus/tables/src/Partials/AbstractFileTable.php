...
abstract class AbstractFileTable extends AbstractTable
{
    ...
    private ?array $indexedValues = null;

    private ?array $normalizedExpectedColumnHeader = null;

    private ?array $columnsHeader = null;
    ...
    private function fetchDataFromFile(string $dataSourceFile): array
    {
        ...
        do {
            ...
            if ($row !== false && $row !== []) { // otherwise skip empty row
                $data[] = $row;
            }
        } while (is_array($row));
    ...
    private function normalizeAndIndex(array $rawData): array
    {
        ...
        return $this->indexValues($formattedValues, $rowsHeader, $this->columnsHeader);
    }
    ...
    private function parseRowsHeader(array $data): array
    {
        ...
        foreach ($data as $rowIndex => $dataRow) {
            ...
            foreach (array_keys($rowsHeaderNamesRow) as $dataColumnIndex) {
    ...
    private function formatValues(array $data): array
    {
        return array_map(
            fn(array $row) => array_map(
                fn($value, $columnIndex) => $this->parseRowValue($value, $columnIndex),
                $row, array_keys($row)
            ),
            $data
        );
    ...
    private function getNormalizedExpectedColumnsHeader(): array
    {
        if ($this->normalizedExpectedColumnHeader === null) {
            ...
            if ($expectedDataHeaderNamesToTypes === []) {
